...
public class ProductDB {
    static public List<Product> getProductList(int receivedId){
        ...
        while(cursor.moveToNext()){
            ...
            productList.add(new Product(id, listName,price, selected == 1, idRelation));
        }
        cursor.close();
        singleTone.closeDb();
        return productList;
    ...
    static public void createProduct(String name, double price, int listId){ // "insert into product (name , price, listId) values (?, ? ,?)"
